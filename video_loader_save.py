...
class VideoLoader:
    ...
    def write(self, filename):
        ...
        self.torch = False
        if self.gray:
            writer = cv2.VideoWriter(filename, cv2.VideoWriter_fourcc(*"MP4V"), self.fps, (self.height, self.width), 0)
        else:
            writer = cv2.VideoWriter(filename, cv2.VideoWriter_fourcc(*"MP4V"), self.fps, (self.height, self.width))
        ...
        while cap.isOpened():
            ...
            if ret:
                frame = np.transpose(frame, axes=[1,0, 2])
                writer.write(self.frame_transform(frame))
                current_frame += 1
            else:
                cap.release()
