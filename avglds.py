...
if __name__ == "__main__":
    ...
    n = 10
    Aa = np.load('data_generated/Aa.pkl', allow_pickle=True)
    m = Aa.shape[-1]
    Ca = np.load('data_generated/Ca.pkl', allow_pickle=True)
    Ahat = np.load('data_generated/Ahat.pkl', allow_pickle=True)
    Chat = np.load('data_generated/Chat.pkl', allow_pickle=True)
    print(np.linalg.norm(Aa, axis=(0,1), ord=2), np.linalg.norm(Ahat, ord=2))
    ...
    for kk in range(maxiter):
        ...
        # exit condition
        if kk>=1:
            print([martin_dist((Ca[:,:,mm], Aa[:,:,mm]), (Chat, Ahat)) for mm in range(m)])
